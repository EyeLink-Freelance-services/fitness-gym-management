...
export function Modal({
  ...
  const modalContent = (
    ...
    >
      <div
        className={`relative flex max-h-[90vh] w-full ${maxWidth ?? "max-w-xl"} flex-col overflow-hidden rounded-[28px] border border-stroke/70 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.25)] backdrop-blur-xl dark:border-dark-3 dark:bg-dark-2/90`}
      ...
      >
        ...
        <div className={`relative flex min-h-0 flex-1 flex-col ${!hideHeader && "px-5 py-5"} text-dark dark:text-dark-6`}>
          {children}
        </div>
      </div>
    </div>
