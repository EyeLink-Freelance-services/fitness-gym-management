...
export async function requestPasswordReset(payload: {
  ...
  const res = await fetch(endpoint(AUTH_CONFIG.passwordResetStartPath), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!res.ok) {
    const data = await parseBody<ApiErrorBody>(res);
    throw new Error(errorMessage(data, "Could not start password reset"));
  }
}
...
export async function confirmPasswordReset(payload: {
  ...
  const res = await fetch(endpoint(AUTH_CONFIG.passwordResetConfirmPath), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!res.ok) {
    const data = await parseBody<ApiErrorBody>(res);
    throw new Error(errorMessage(data, "Could not reset password"));
  }
}
