...
export type SaveCompanySchemaGroupInput = {
  id?: string;
  name: string;
...
export type SaveCompanySchemaFieldInput = {
  id?: string;
  group_id: string;
  label: string;
  key: string;
...
export type SaveCompanyValidationRuleInput = {
  id?: string;
  title: string;
  description?: string | null;
  value: string;
...
export type SaveCoachFieldOverrideInput = {
  field_id: string;
  label?: string | null;
  unit?: string | null;
  placeholder?: string | null;
...
export type SaveCoachValidationRuleOverrideInput = {
...
export type SaveCoachGroupOverrideInput = {
  group_id: string;
  name?: string | null;
  description?: string | null;
  accent_color?: string | null;
...
export async function saveCoachSchemaOverrideBundleQuery(payload: {
  ...
  const auth = await requirePermission(AuthPermission.settings.view);
...
export async function saveCompanySchemaGroupQuery(
  ...
  const row = {
    ...
    unit_hint: payload.unit_hint ?? null,
...
export async function saveCompanySchemaFieldQuery(
  ...
  const row = {
    ...
    show_portal: payload.show_portal ?? false,
    archived: payload.archived ?? false,
    sort_order: payload.sort_order ?? 0,
...
export async function saveCompanyValidationRuleQuery(
...
export async function saveCoachGroupOverrideQuery(
  ...
  const { data, error } = await supabase
    ...
    .upsert(
      ...
      {
        onConflict: "coach_user_id,group_id",
      },
    )
    .select("*")
    .single();
...
export async function saveCoachFieldOverrideQuery(
    ...
    .upsert(
      {
        ...
        read_only: payload.read_only ?? null,
      ...
      {
        onConflict: "coach_user_id,field_id",
      },
    )
    .select("*")
    .single();
...
export async function saveCoachValidationRuleOverrideQuery(
  ...
  const { data, error } = await supabase
    ...
    .upsert(
      ...
      {
        onConflict: "coach_user_id,validation_rule_id",
      },
    )
    .select("*")
    .single();
